package ml.melun.mangaviewextended;

import static java.lang.System.currentTimeMillis;
import static ml.melun.mangaviewextended.MainApplication.httpClient;
import static ml.melun.mangaviewextended.activity.CaptchaActivity.REQUEST_CAPTCHA;
import static ml.melun.mangaviewextended.activity.SettingsActivity.urlSettingPopup;
...
import ml.melun.mangaviewextended.activity.CaptchaActivity;
import ml.melun.mangaviewextended.activity.EpisodeActivity;
import ml.melun.mangaviewextended.activity.LoginActivity;
import ml.melun.mangaviewextended.activity.ViewerActivity;
import ml.melun.mangaviewextended.activity.ViewerActivity2;
import ml.melun.mangaviewextended.activity.ViewerActivity3;
import ml.melun.mangaviewextended.interfaces.IntegerCallback;
import ml.melun.mangaviewextended.interfaces.StringCallback;
import ml.melun.mangaviewextended.mangaview.CustomHttpClient;
import ml.melun.mangaviewextended.mangaview.Login;
import ml.melun.mangaviewextended.mangaview.Manga;
import ml.melun.mangaviewextended.mangaview.Title;
...
//    }
    ...
    public static void showPopup(Context context, String title, String content, DialogInterface.OnClickListener clickListener, DialogInterface.OnCancelListener cancelListener) {
        ...
        builder.setTitle(title).setMessage(content).setPositiveButton("확인", clickListener).setOnCancelListener(cancelListener).show();
    }

    public static void showYesNoPopup(Context context, String title, String content, DialogInterface.OnClickListener posClickListener, DialogInterface.OnClickListener negClickListener, DialogInterface.OnCancelListener cancelListener) {

        AlertDialog.Builder builder;
        if (new Preference(context).getDarkTheme())
            builder = new AlertDialog.Builder(context, R.style.darkDialog);
        else builder = new AlertDialog.Builder(context);
        builder.setTitle(title).setMessage(content).setPositiveButton("예", posClickListener).setNegativeButton("아니오", negClickListener).setOnCancelListener(cancelListener).show();
    }

    public static void showYesNoPopup(boolean dark, Context context, String title, String content, DialogInterface.OnClickListener posClickListener, DialogInterface.OnClickListener negClickListener, DialogInterface.OnCancelListener cancelListener) {
    ...
    public static void showYesNoNeutralPopup(Context context, String title, String content, String neutral, DialogInterface.OnClickListener posClickListener, DialogInterface.OnClickListener negClickListener, DialogInterface.OnClickListener neuClickListener, DialogInterface.OnCancelListener cancelListener) {

        AlertDialog.Builder builder;
        if (new Preference(context).getDarkTheme())
            builder = new AlertDialog.Builder(context, R.style.darkDialog);
        else builder = new AlertDialog.Builder(context);
        builder.setTitle(title).setMessage(content).setPositiveButton("예", posClickListener).setNegativeButton("아니오", negClickListener).setNeutralButton(neutral, neuClickListener).setOnCancelListener(cancelListener).show();
    }
    ...
    public static void showErrorPopup(Context context, String message, Exception e, boolean force_close) {
        ...
        builder.setTitle(title).setMessage(message).setPositiveButton("확인", (dialog, which) -> {
    ...
    public static boolean checkConnection(Context context) {
        if (context != null) {
            ConnectivityManager connectivityManager = (ConnectivityManager) ((Activity) context).getSystemService(Context.CONNECTIVITY_SERVICE);
    ...
    public static void showCaptchaPopup(Context context, int code, Exception e, boolean force_close, Fragment fragment, Preference p) {
        if (context != null) {
            if (!checkConnection(context)) {
                ...
                builder.setTitle(title).setMessage(content).setNeutralButton("확인", (dialogInterface, i) -> {
                    if (force_close) ((Activity) context).finish();
                }).setPositiveButton("CAPTCHA 인증", (dialog, which) -> startCaptchaActivity(context, code, fragment)).setNegativeButton("URL 설정", (dialogInterface, i) -> urlSettingPopup(context, p)).setOnCancelListener(dialogInterface -> {
                    if (force_close) ((Activity) context).finish();
                });
                if (e != null) {
                    builder.setNeutralButton("자세히", (dialog, which) -> showStackTrace(context, e));
    ...
    static void startCaptchaActivity(Context context, int code, Fragment fragment) {
        ...
        if (fragment == null) ((Activity) context).startActivityForResult(captchaIntent, code);
        else fragment.startActivityForResult(captchaIntent, code);
    }
    ...
    public static void showTokiCaptchaPopup(Context context, Preference p) {
        ...
        new Thread(() -> {
            ...
            try {
                ...
                ((Activity) context).runOnUiThread(() -> Glide.with(img).load(b).into(img));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }).start();

        builder.setTitle(title).setView(v).setPositiveButton("확인", (dialog, which) -> new Thread(() -> {
            RequestBody requestBody = new FormBody.Builder().addEncoded("url", p.getUrl()).addEncoded("captcha_key", answer.getText().toString()).build();
        ...
        }).start()).setNegativeButton("취소", (dialogInterface, i) -> ((Activity) context).finish()).setOnCancelListener(dialogInterface -> ((Activity) context).finish());
    ...
    private static void showStackTrace(Context context, Exception e) {
        ...
        if (e.getMessage() != null) sbuilder.append(e.getMessage()).append("\n");
        ...
        builder.setTitle(title).setMessage(error).setNeutralButton("복사", (dialog, which) -> {
            ClipboardManager clipboard = (ClipboardManager) context.getSystemService(Context.CLIPBOARD_SERVICE);
            ClipData clip = ClipData.newPlainText("stack_trace", error);
            clipboard.setPrimaryClip(clip);
            Toast.makeText(context, "클립보드에 복사되었습니다.", Toast.LENGTH_SHORT).show();
            ((Activity) context).finish();
        }).setPositiveButton("확인", (dialog, which) -> ((Activity) context).finish()).setOnCancelListener(dialog -> ((Activity) context).finish()).show();
    }
    ...
    public static void showPopup(Context context, String title, String content) {
        ...
        builder.setTitle(title).setMessage(content).setPositiveButton("확인", null).show();
    }
...
//                    +"&comment_id=&pim=&sca=&sfl=&stx=&spt=&page=&is_good=0&wr_content="+URLEncoder.encode(content, "UTF-8");
            RequestBody requestBody = new FormBody.Builder().addEncoded("token", token).addEncoded("w", "c").addEncoded("bo_table", "manga").addEncoded("wr_id", String.valueOf(id)).addEncoded("comment_id", "").addEncoded("pim", "").addEncoded("sca", "").addEncoded("sfl", "").addEncoded("stx", "").addEncoded("spt", "").addEncoded("page", "").addEncoded("is_good", "0").addEncoded("wr_content", content).build();
    ...
    public static int getNumberFromString(String input) {
        ...
        for (int i = 0; i < input.length(); i++) {
            if (Character.digit(input.charAt(i), 10) < 0) {
                if (i > 0) return Integer.parseInt(input.substring(0, i));
                else return -1;
            }
        }
        return -1;
